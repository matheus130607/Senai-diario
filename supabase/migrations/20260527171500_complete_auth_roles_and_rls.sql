-- Consolida o banco remoto para os perfis reais da aplicacao:
-- Coordenacao, Secretaria, Professor, Empresa e TIC.
--
-- Esta migration e idempotente e tambem remove policies legadas abertas para
-- anon que existiam no projeto piloto.
...
begin
  ...
  loop
    execute format('alter table public.turmas drop constraint %I', constraint_row.conname);
  end loop;

  for constraint_row in
    select conname
    from pg_constraint
    where conrelid = 'public.presencas'::regclass
      and contype = 'c'
      and pg_get_constraintdef(oid) ilike '%status%'
  loop
    execute format('alter table public.presencas drop constraint %I', constraint_row.conname);
  end loop;
end $$;

alter table public.turmas
  add constraint turmas_status_check
  check (status in ('Ativo', 'Pausado', 'Concluido', 'Inativo'));

alter table public.presencas
  add constraint presencas_status_check
  check (status in ('presente', 'falta', 'pendente', 'Presente', 'Faltou', 'Pendente'));
...
begin
  for constraint_row in
    select conname
    from pg_constraint
    where conrelid = 'public.administradores'::regclass
      and contype = 'c'
      and pg_get_constraintdef(oid) ilike '%perfil%'
  loop
    execute format('alter table public.administradores drop constraint %I', constraint_row.conname);
  end loop;
end $$;

alter table public.administradores
  add constraint administradores_perfil_check
  check (perfil in ('coordenacao', 'secretaria'));

update public.administradores
set perfil = 'coordenacao',
    updated_at = now()
where perfil is null or perfil = '' or perfil = 'admin';

insert into public.administradores (nome, email, senha_hash, perfil)
values ('Secretaria Academica', 'secretaria@senaisp.edu.br', 'senha_teste_123', 'secretaria')
...
create unique index if not exists idx_professores_turmas_unique on public.professores_turmas (professor_id, turma_id);
...
insert into public.user_profiles (auth_user_id, role, nome, email, status)
select u.id, a.perfil, a.nome, lower(a.email), 'active'
from auth.users u
join public.administradores a on lower(a.email) = lower(u.email)
where a.perfil in ('coordenacao', 'secretaria')
on conflict (email) do update
...
insert into public.user_profiles (auth_user_id, role, nome, email, status, related_professor_id)
select u.id, 'professor', p.nome, lower(p.email_institucional), 'active', p.id
from auth.users u
join public.professores p on lower(p.email_institucional) = lower(u.email)
on conflict (email) do update
set auth_user_id = excluded.auth_user_id,
    role = excluded.role,
    nome = excluded.nome,
    status = excluded.status,
    related_professor_id = excluded.related_professor_id,
    updated_at = now();

insert into public.user_profiles (auth_user_id, role, nome, email, status, related_empresa_id)
select u.id, 'empresa', e.nome, lower(e.email), 'active', e.id
from auth.users u
join public.empresas e on lower(e.email) = lower(u.email)
on conflict (email) do update
set auth_user_id = excluded.auth_user_id,
    role = excluded.role,
    nome = excluded.nome,
    status = excluded.status,
    related_empresa_id = excluded.related_empresa_id,
    updated_at = now();

insert into public.user_profiles (auth_user_id, role, nome, email, status)
select u.id, 'tic', 'TIC Super Admin', lower(u.email), 'active'
from auth.users u
where lower(u.email) = 'tic@senaisp.edu.br'
on conflict (email) do update
set auth_user_id = excluded.auth_user_id,
    role = excluded.role,
    nome = excluded.nome,
...
create policy "turmas_select" on public.turmas for select to authenticated using (
  public.current_app_role() in ('coordenacao', 'secretaria', 'tic')
  or exists (
    select 1
    from public.professores p
    join public.professores_turmas pt on pt.professor_id = p.id
    where pt.turma_id = turmas.id
      and lower(p.email_institucional) = public.current_user_email()
  )
  or exists (
    select 1
    from public.empresas e
    join public.alunos a on a.empresa_id = e.id
...
create policy "alunos_select" on public.alunos for select to authenticated using (
  ...
  or exists (
    ...
    from public.professores p
    join public.professores_turmas pt on pt.professor_id = p.id
    where pt.turma_id = alunos.turma_id
      and lower(p.email_institucional) = public.current_user_email()
  )
  or exists (
    select 1
    from public.empresas e
    where e.id = alunos.empresa_id
      and lower(e.email) = public.current_user_email()
  )
);
...
create policy "professores_select" on public.professores for select to authenticated using (
  public.current_app_role() in ('coordenacao', 'secretaria', 'tic')
  or lower(email_institucional) = public.current_user_email()
);
...
create policy "administradores_select" on public.administradores for select to authenticated using (
  public.current_app_role() in ('coordenacao', 'secretaria', 'tic')
  or lower(email) = public.current_user_email()
);
...
create policy "professores_turmas_select" on public.professores_turmas for select to authenticated using (
  public.current_app_role() in ('coordenacao', 'secretaria', 'tic')
  or exists (
    select 1
    from public.professores p
    where p.id = professores_turmas.professor_id
      and lower(p.email_institucional) = public.current_user_email()
  )
);
...
create policy "presencas_select" on public.presencas for select to authenticated using (
  public.current_app_role() in ('coordenacao', 'secretaria', 'tic')
  or exists (
    select 1
    from public.professores p
    join public.professores_turmas pt on pt.professor_id = p.id
    where pt.turma_id = presencas.turma_id
      and lower(p.email_institucional) = public.current_user_email()
  )
  or exists (
    select 1
...
create policy "presencas_insert" on public.presencas for insert to authenticated with check (
  public.current_app_role() in ('secretaria', 'tic')
  or exists (
    select 1
    from public.professores p
    join public.professores_turmas pt on pt.professor_id = p.id
...
create policy "presencas_update" on public.presencas for update to authenticated using (
  public.current_app_role() in ('secretaria', 'tic')
  or exists (
    select 1
    from public.professores p
...
create policy "user_profiles_select" on public.user_profiles for select to authenticated using (
  ...
  or public.current_app_role() in ('secretaria', 'tic')
);
...
create policy "user_profiles_update" on public.user_profiles for update to authenticated using (
  auth_user_id = auth.uid()
  or public.current_app_role() = 'tic'
) with check (
  auth_user_id = auth.uid()
  or public.current_app_role() = 'tic'
);
...
create policy "user_preferences_select" on public.user_preferences for select to authenticated using (
  user_id = public.current_profile_id()::text
  or public.current_app_role() = 'tic'
);
create policy "user_preferences_insert" on public.user_preferences for insert to authenticated with check (
  user_id = public.current_profile_id()::text
  or public.current_app_role() = 'tic'
);
create policy "user_preferences_update" on public.user_preferences for update to authenticated using (
  user_id = public.current_profile_id()::text
  or public.current_app_role() = 'tic'
) with check (
  user_id = public.current_profile_id()::text
