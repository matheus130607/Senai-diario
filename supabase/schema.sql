...
-- Politicas finais para piloto com Supabase Auth.
-- As politicas abertas acima preservam compatibilidade de bancos antigos durante
-- aplicacao manual; esta secao substitui o acesso anonimo pelo RBAC real.
...
create policy "turmas_select" on public.turmas for select to authenticated using (
  ...
  or exists (
    ...
    where pt.turma_id = turmas.id and lower(p.email_institucional) = public.current_user_email()
  )
  or exists (
    select 1
    from public.empresas e
    join public.alunos a on a.empresa_id = e.id
    where a.turma_id = turmas.id and lower(e.email) = public.current_user_email()
  )
);
...
create policy "alunos_select" on public.alunos for select to authenticated using (
  ...
  or exists (
    ...
    from public.professores p
    join public.professores_turmas pt on pt.professor_id = p.id
    where pt.turma_id = alunos.turma_id and lower(p.email_institucional) = public.current_user_email()
  )
  or exists (
    select 1
    from public.empresas e
    where e.id = alunos.empresa_id and lower(e.email) = public.current_user_email()
  )
);
...
create policy "professores_turmas_select" on public.professores_turmas for select to authenticated using (
  ...
  or exists (
    ...
    where p.id = professores_turmas.professor_id and lower(p.email_institucional) = public.current_user_email()
  )
);
...
create policy "presencas_select" on public.presencas for select to authenticated using (
  ...
  or exists (
    ...
    from public.professores p
    join public.professores_turmas pt on pt.professor_id = p.id
    where pt.turma_id = presencas.turma_id and lower(p.email_institucional) = public.current_user_email()
  )
  or exists (
    select 1
    from public.empresas e
    join public.alunos a on a.empresa_id = e.id
    where a.id = presencas.aluno_id and lower(e.email) = public.current_user_email()
  )
);
